...
//foolant
    ...
    if ( empty( $theme_root ) ) {
        ...
        if ( false === $theme_root ){
            $theme_root = realpath(WP_CONTENT_DIR . '/themes') ?: WP_CONTENT_DIR;
        }



		elseif ( ! in_array( $theme_root, (array) $wp_theme_directories ) ){
            $theme_root = realpath(WP_CONTENT_DIR . $theme_root) ?: WP_CONTENT_DIR;
        }
	...
	if ( $cache_expiration = apply_filters( 'wp_cache_themes_persistently', false, 'search_theme_directories' ) ) {
		...
		if ( is_array( $cached_roots ) ) {
			foreach ( $cached_roots as $theme_dir => $theme_root ) {
                ...
                //foolant
				if ( ! isset( $relative_theme_roots[ $theme_root ] ) || !is_file($theme_dir . '/style.css'))
					continue;

...
class Blade {
	...
	{
		$cachepath = static::compiled($path);
		return !file_exists($cachepath) or filemtime($path) > filemtime($cachepath);
	}
	...
	protected static function compile_layouts($value)
	{
		...
		//Strip end of file
		$value = trim($value);// rtrim($value); # foolant
	...
	public static function compiled($path)
	{
		...
		return $blade_storage_path.'/'.md5($path);
	}
